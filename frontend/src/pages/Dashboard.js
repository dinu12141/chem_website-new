  ...
  const fetchDashboardData = async () => {
    try {
      const [announcementsRes, coursesRes] = await Promise.all([
        axios.get(`${API}/announcements/${user.al_year}`),
        axios.get(`${API}/courses`)
      ]);
      
      setAnnouncements(announcementsRes.data.slice(0, 5));
      setCourses(coursesRes.data.filter(course => course.year === user.al_year));
    } catch (error) {
            ...
            <Card className="bg-gray-800 border-gray-700 mb-6">
              ...
              <CardContent className="space-y-4">
                <div className="text-center mb-6">
                  ...
                  <p className="text-orange-500 font-mono text-lg">{user?.register_number}</p>
                </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white">{user?.al_year}</p>
                    </div>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-white flex items-center">
                  ...
                  Available Courses for A/L {user?.al_year}
                </CardTitle>
              </CardHeader>
              <CardContent>
