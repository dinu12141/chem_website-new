...
const FirebaseAdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      switch (activeTab) {
        case 'messages':
          const messagesSnapshot = await getDocs(collection(db, 'support_messages'));
        ...
        case 'classes':
          ...
          break;
        case 'telegram':
          const telegramSnapshot = await getDocs(collection(db, 'telegram_channels'));
          const telegramData = telegramSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          setTelegramChannels(telegramData);
          break;
        case 'videos':
          const videosSnapshot = await getDocs(collection(db, 'video_lessons'));
  ...
  const resolveMessage = async (messageId) => {
    try {
      ...
      await updateDoc(messageRef, { is_resolved: !message.is_resolved });
  ...
  const createTelegramChannel = async () => {
    try {
      await addDoc(collection(db, 'telegram_channels'), newTelegramChannel);
  ...
  const updateTelegramChannel = async () => {
    try {
      const channelRef = doc(db, 'telegram_channels', editingTelegramChannel.id);
      await updateDoc(channelRef, editingTelegramChannel);
      setTelegramChannels(telegramChannels.map(channel => 
        channel.id === editingTelegramChannel.id ? editingTelegramChannel : channel
      ));
  ...
  const createVideoLesson = async () => {
    try {
      await addDoc(collection(db, 'video_lessons'), newVideoLesson);
      setNewVideoLesson({
        title: '', title_sinhala: '', description: '', video_url: '', 
        thumbnail: '', duration: '', course_id: '', is_published: true
      });
  ...
  const updateVideoLesson = async () => {
    try {
      const lessonRef = doc(db, 'video_lessons', editingVideoLesson.id);
      await updateDoc(lessonRef, editingVideoLesson);
      setVideoLessons(videoLessons.map(lesson => 
        lesson.id === editingVideoLesson.id ? editingVideoLesson : lesson
      ));
  ...
  const deleteVideoLesson = async (lessonId) => {
    ...
    try {
      await deleteDoc(doc(db, 'video_lessons', lessonId));
                            ...
                            {admins.map((admin) => (
                              <tr key={admin.id} className="border-b border-gray-700 hover:bg-gray-750">
                                ...
                                <td className="px-4 py-3">
                                  {admin.created_at ? new Date(admin.created_at.toDate()).toLocaleDateString() : 'N/A'}
                                </td>
                                <td className="px-4 py-3">
                                  <Button
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-gray-400 text-sm mt-1">
                                  From {message.name} • {message.created_at ? formatDate(message.created_at.toDate()) : 'N/A'}
                                </p>
                              </div>
