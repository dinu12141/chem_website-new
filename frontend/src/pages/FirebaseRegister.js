...
import { Loader2, User, Mail, Phone, CreditCard, GraduationCap, School, Lock, CheckCircle, Eye, EyeOff } from 'lucide-react';
...
const FirebaseRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // For Firebase registration, we only need email and password
    const result = await register(formData.email, formData.password);
    
    if (result.success) {
      // In a real implementation, you would also store additional user data in Firestore
      setSuccess({
        student_name: formData.fullName,
        register_number: 'FB' + Date.now() // Generate a Firebase-based register number
      });
      setStep(2);
    } else {
